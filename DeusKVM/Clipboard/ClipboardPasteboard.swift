...
final class ClipboardPasteboard: @unchecked Sendable {
    ...
    func readFile(epoch: UInt32, sequence: UInt32, offset: UInt32, reply: @escaping @Sendable (Data?) -> Void) {
        queue.async { [self] in
            reply(fileBytes(epoch: epoch, sequence: sequence, offset: offset, count: ClipboardTransfer.blockBytes))
        }
    }
    ...
    private func poll() {
        ...
        board.clearContents()
        if board.writeObjects([item]) { versions.imported(board.changeCount); pending = nil; file = nil; network.revoke() }
    }
